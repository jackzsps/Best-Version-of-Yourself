...
import {
  VintageInput, VintageBottomSheetSelect, VintageTextArea,
  BentoInput, BentoBottomSheetSelect, BentoTextArea,
        ...
        <div className="space-y-6">
          <div className={`flex p-1 ${isVintage ? 'border-b border-vintage-line pb-4 mb-2' : 'bg-gray-100 rounded-xl'}`}>
            {entryTypes.map(type => (
              <button
                ...
                className={`flex-1 py-2 text-xs font-bold transition-all ${recordType === type
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="col-span-2">
              ...
              {isVintage
                ? <VintageBottomSheetSelect title={t.addEntry.category} value={category} onChange={val => setCategory(val as ExpenseCategory)} options={categories.map(c => ({ value: c, label: t.categories[c] }))} />
                : <BentoBottomSheetSelect title={t.addEntry.category} value={category} onChange={val => setCategory(val as ExpenseCategory)} options={categories.map(c => ({ value: c, label: t.categories[c] }))} />
              }
            </div>
                ...
                <div className="flex gap-2">
                  {usageCategories.map(u => (
                    <button
                      key={u}
                      onClick={() => setUsage(u)}
                      className={`flex-1 py-3 text-sm rounded-xl border transition-all ${isVintage ? 'rounded-none border-2 font-typewriter' : ''
            ...
            {(recordType === 'expense' || recordType === 'combined') && (
              <div>
                ...
                {isVintage
                  ? <VintageBottomSheetSelect title={t.addEntry.paymentMethod} value={paymentMethod} onChange={val => setPaymentMethod(val as PaymentMethod)} options={paymentMethods.map(p => ({ value: p, label: t.paymentMethods[p] }))} />
                  : <BentoBottomSheetSelect title={t.addEntry.paymentMethod} value={paymentMethod} onChange={val => setPaymentMethod(val as PaymentMethod)} options={paymentMethods.map(p => ({ value: p, label: t.paymentMethods[p] }))} />
                }
              </div>
            )}
